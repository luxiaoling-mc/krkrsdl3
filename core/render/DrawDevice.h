...
public:
    ...
    //!				通常、ここでは delete this
    //!を実行し、描画デバイスを破棄するが、その前に 				AddLayerManager()
    //!でこの描画デバイスの管理下に入っている 				レイヤマネージャをすべて Release する。
    ...
    //!				レイヤマネージャをロックすること。
    virtual void AddLayerManager(iTVPLayerManager* manager) = 0;

    //! @brief		(Window→DrawDevice) レイヤマネージャを削除する
    //! @note		プライマリレイヤが invalidate される際に呼び出される。
    // TODO: プライマリレイヤ無効化、あるいはウィンドウ破棄時の終了処理が正しいか？
    virtual void RemoveLayerManager(iTVPLayerManager* manager) = 0;
    ...
    //	virtual void SetTargetWindow(int wnd, bool is_main) = 0;
    ...
    //!				この矩形は、GetSrcSize で返した値に対し、Window.zoomNumer や
    //!Window.zoomDenum 				プロパティによる拡大率や、Window.layerLeft や Window.layerTop が加味された
    //!				矩形である。
    ...
    //!				際に呼び出される
    virtual void NotifyLayerResize(iTVPLayerManager* manager) = 0;
    ...
    //!				それまでの変更分がすべて描画される。
    virtual void NotifyLayerImageChange(iTVPLayerManager* manager) = 0;

    //---- ユーザーインターフェース関連
    //! @brief		(Window→DrawDevice) クリックされた
    //! @param		x		描画矩形内における x 位置(描画矩形の左上が原点)
    //! @param		y		描画矩形内における y 位置(描画矩形の左上が原点)
    virtual void OnClick(tjs_int x, tjs_int y) = 0;

    //! @brief		(Window→DrawDevice) ダブルクリックされた
    //! @param		x		描画矩形内における x 位置(描画矩形の左上が原点)
    //! @param		y		描画矩形内における y 位置(描画矩形の左上が原点)
    virtual void OnDoubleClick(tjs_int x, tjs_int y) = 0;
    ...
    virtual void OnMouseDown(tjs_int x, tjs_int y, tTVPMouseButton mb, tjs_uint32 flags) = 0;
    ...
    //! @param		mb		どのマウスボタンか
    //! @param		flags	フラグ(TVP_SS_*定数の組み合わせ)
    virtual void OnMouseUp(tjs_int x, tjs_int y, tTVPMouseButton mb, tjs_uint32 flags) = 0;

    //! @brief		(Window→DrawDevice) マウスが移動した
    //! @param		x		描画矩形内における x 位置(描画矩形の左上が原点)
    //! @param		y		描画矩形内における y 位置(描画矩形の左上が原点)
    //! @param		flags	フラグ(TVP_SS_*定数の組み合わせ)
    virtual void OnMouseMove(tjs_int x, tjs_int y, tjs_uint32 flags) = 0;
    ...
    virtual void OnKeyDown(tjs_uint key, tjs_uint32 shift) = 0;

    //! @brief		(Window→DrawDevice) キーが離された
    //! @param		key		仮想キーコード
    //! @param		shift	シフトキーの状態
    virtual void OnKeyUp(tjs_uint key, tjs_uint32 shift) = 0;
    ...
    //! @param		x		描画矩形内における x 位置(描画矩形の左上が原点)
    //! @param		y		描画矩形内における y 位置(描画矩形の左上が原点)
    virtual void OnMouseWheel(tjs_uint32 shift, tjs_int delta, tjs_int x, tjs_int y) = 0;
    ...
    //! @param		cy		触れている高さ
    //! @param		id		タッチ識別用ID
    virtual void OnTouchDown(tjs_real x, tjs_real y, tjs_real cx, tjs_real cy, tjs_uint32 id) = 0;
    ...
    //! @param		cy		触れている高さ
    //! @param		id		タッチ識別用ID
    virtual void OnTouchUp(tjs_real x, tjs_real y, tjs_real cx, tjs_real cy, tjs_uint32 id) = 0;
    ...
    //! @param		cy		触れている高さ
    //! @param		id		タッチ識別用ID
    virtual void OnTouchMove(tjs_real x, tjs_real y, tjs_real cx, tjs_real cy, tjs_uint32 id) = 0;
    ...
    //! @param		cy		触れている高さ
    //! @param		flag	タッチ状態フラグ
    virtual void OnTouchScaling(
        tjs_real startdist, tjs_real curdist, tjs_real cx, tjs_real cy, tjs_int flag) = 0;
    ...
    virtual void OnDisplayRotate(
        tjs_int orientation, tjs_int rotate, tjs_int bpp, tjs_int width, tjs_int height) = 0;
    ...
    virtual void SetDefaultMouseCursor(iTVPLayerManager* manager) = 0;

    //! @brief		(LayerManager→DrawDevice) マウスカーソルの形状を設定する
    //! @param		manager		レイヤマネージャ
    //! @param		cursor		マウスカーソル形状番号
    virtual void SetMouseCursor(iTVPLayerManager* manager, tjs_int cursor) = 0;
    ...
    virtual void GetCursorPos(iTVPLayerManager* manager, tjs_int& x, tjs_int& y) = 0;
    ...
    //! @param		y プライマリレイヤ上の座標におけるマウスカーソルのy位置
    //! @note		座標はプライマリレイヤ上の座標なので、必要ならば変換を行う
    virtual void SetCursorPos(iTVPLayerManager* manager, tjs_int x, tjs_int y) = 0;
    ...
    //!::ReleaseCapture() などで 				マウスのキャプチャを開放する。
    virtual void WindowReleaseCapture(iTVPLayerManager* manager) = 0;
    ...
    //!				表示したり、未確定の文字をそこに表示したりする際のフォントは layer
    //!パラメータ 				で示されるレイヤが持つ情報によるが、プラグインからその情報を得たり設定したり
    //!				するインターフェースは今のところない。
    //! @note		座標はプライマリレイヤ上の座標なので、必要ならば変換を行う。
    ...
    //! @param		manager		レイヤマネージャ
    virtual void DisableAttentionPoint(iTVPLayerManager* manager) = 0;

    //! @brief		(LayerManager→DrawDevice) IMEモードの設定
    //! @param		manager		レイヤマネージャ
    //! @param		mode		IMEモード
    virtual void SetImeMode(iTVPLayerManager* manager, tTVPImeMode mode) = 0;

    //! @brief		(LayerManager→DrawDevice) IMEモードのリセット
    //! @param		manager		レイヤマネージャ
    virtual void ResetImeMode(iTVPLayerManager* manager) = 0;
    ...
    //!の間に複数回呼ばれる可能性がある。 				基本的には、bits と bitmapinfo
    //!で表されるビットマップのうち、cliprect で 				示される矩形を x, y
    //!位置に転送すればよいが、描画矩形の大きさに合わせた
    ...
    virtual void SetShowUpdateRect(bool b) = 0;
    virtual void Clear() {}
    ...
    virtual bool SwitchToFullScreen(int window,
                                    tjs_uint w,
                                    tjs_uint h,
                                    tjs_uint bpp,
                                    tjs_uint color,
    ...
    virtual void RevertFromFullScreen(
        int window, tjs_uint w, tjs_uint h, tjs_uint bpp, tjs_uint color) = 0;

    //! @brief		(Window->DrawDevice) VBlank待ちを行う
    //! @param		in_vblank	待たなくてもVBlank内だったかどうかを返す( !0 : 内、0: 外 )
    //! @param		delayed		1フレーム遅延が発生したかどうかを返す( !0 : 発生、0:
    //! 発生せず )
    //! @return		Wait可不可 true : 可能、false : 不可
    virtual bool WaitForVBlank(tjs_int* in_vblank, tjs_int* delayed) = 0;
};
//---------------------------------------------------------------------------
/*]*/
...
//---------------------------------------------------------------------------
class tTVPBasicDrawDevice : public iTVPDrawDevice
{
...
public:
    ...
    virtual void OnMouseDown(tjs_int x, tjs_int y, tTVPMouseButton mb, tjs_uint32 flags);
    virtual void OnMouseUp(tjs_int x, tjs_int y, tTVPMouseButton mb, tjs_uint32 flags);
    ...
    virtual void SetHintText(iTVPLayerManager* manager, iTJSDispatch2* sender, const ttstr& text);
