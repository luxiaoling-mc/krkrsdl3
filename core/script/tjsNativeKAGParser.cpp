...
const tjs_char* TVPKAGNoLine = TJS_N("Readed scenario file %1 is empty.");
const tjs_char* TVPKAGCannotOmmitFirstLabelName =
    TJS_N("Can not ommit first label name in scenario.");
// const tjs_char* TVPInternalError = TJS_N("内部エラーが発生しました: at %1 line %2");
const tjs_char* TVPKAGMalformedSaveData = TJS_N("Malformed savedata, data may damaged.");
const tjs_char* TVPKAGLabelNotFound = TJS_N("Label %2 not found in scenario file %1.");
const tjs_char* TVPLabelOrScriptInMacro = TJS_N("Label in macro 'iscript' is illegal.");
const tjs_char* TVPKAGInlineScriptNotEnd = TJS_N("Matched [endscript] or @endscript not found.");
const tjs_char* TVPKAGSyntaxError = TJS_N("Syntax error.'[' match to ']', \" match to \", 'macro' "
                                          "march to 'endmacro'. Notice space and newline.");
const tjs_char* TVPKAGCallStackUnderflow =
    TJS_N("'return' is not matched to any 'call' ( 'return' is unexpected )");
const tjs_char* TVPKAGReturnLostSync =
    TJS_N("Lost return position due to the scenario file changed.");
const tjs_char* TVPKAGSpecifyKAGParser = TJS_N("Please specify KAGParser object.");
const tjs_char* TVPUnknownMacroName = TJS_N("Unknown macro \"%1\"");
...
typedef tTJSHashCache<ttstr,
    ...
    tTVPScenarioCache;
static tTVPScenarioCache TVPScenarioCache(TVP_SCENARIO_MAX_CACHE_SIZE);
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
tjs_error tTJSNI_KAGParser::Construct(tjs_int numparams,
                                      tTJSVariant** param,
...

    {
        ...
        if (val.Type() != tvtVoid)
        {
            ...
            for (tjs_int i = 0; i < count; i++)
            {
                ...
                CallStack.push_back(tCallStackData(Storage, Label, Offset, OrgLineStr, LineBuffer,
                                                   Pos, LineBufferUsing, MacroArgStackBase,
                                                   MacroArgStackDepth, ExcludeLevelStack,
...
//---------------------------------------------------------------------------
void tTJSNI_KAGParser::GoToStorageAndLabel(const ttstr& storage, const ttstr& label)
{
    if (!storage.IsEmpty())
        LoadScenario(storage);
    if (!label.IsEmpty())
...
{
    for (std::vector<iTJSDispatch2*>::iterator i = MacroArgs.begin(); i != MacroArgs.end(); i++)
    {
        (*i)->Release();
    }
    MacroArgs.clear();
...
//---------------------------------------------------------------------------
void tTJSNI_KAGParser::FindNearestLabel(tjs_int start, tjs_int& labelline, ttstr& labelname)
{
...
void tTJSNI_KAGParser::PushCallStack()
{
    ...
    CallStack.push_back(tCallStackData(StorageName, labelname, CurLine - labelline, curline_content,
                                       LineBuffer, CurPos, LineBufferUsing, MacroArgStackBase,
                                       MacroArgStackDepth, ExcludeLevelStack, ExcludeLevel,
                                       IfLevelExecutedStack, IfLevel));
...
parse_start:
    ...
    while (true)
    {
        ...
        if (Interrupted)
        {
            ...
            DicObj->PropSetByVS(TJS_MEMBERENSURE, __tag_name.AsVariantStringNoAddRef(), &r_val,
                                DicObj);
            Interrupted = false;
            DicObj->AddRef();
            return DicObj;
        ...
        if (!IgnoreCR)
        {
            if ((CurLineStr[CurPos] == TJS_N('\\') && CurLineStr[CurPos + 1] == 0) ||
                (CurLineStr[CurPos] == 0 && CurPos >= 3 && CurLineStr[CurPos - 3] == TJS_N('[') &&
                 CurLineStr[CurPos - 2] == TJS_N('p') && // for line ending with [p]
            ...
            if (CurLineStr[CurPos] == 0)
            {
                ...
                DicObj->PropSetByVS(TJS_MEMBERENSURE, __tag_name.AsVariantStringNoAddRef(), &r_val,
                                    DicObj);
                DicObj->PropSetByVS(TJS_MEMBERENSURE, __eol_name.AsVariantStringNoAddRef(),
                                    &true_val, DicObj);
                if (RecordingMacro)
        ...
        else
        {
            if (CurLineStr[CurPos] != TJS_N('[') ||
                CurLineStr[CurPos] == TJS_N('[') && CurLineStr[CurPos + 1] == TJS_N('['))
            {
                // normal character
                ...
                else if (chLen > 0 && CurLineStr[CurPos] != TJS_N('\n'))
                {
                    ...
                    DicObj->PropSetByVS(TJS_MEMBERENSURE, __tag_name.AsVariantStringNoAddRef(),
                                        &tag_val, DicObj);
                    tTJSVariant ch_val(ttstr(CurLineStr + CurPos, chLen));
                    static ttstr text_name(TJSMapGlobalStringMap(TJS_N("text")));
                    DicObj->PropSetByVS(TJS_MEMBERENSURE, text_name.AsVariantStringNoAddRef(),
                                        &ch_val, DicObj);
        ...
        while (CurLineStr[CurPos] && !TVPIsWS(CurLineStr[CurPos]) && CurLineStr[CurPos] != ldelim)
            CurPos++;
        ...
        tagname.ToLowerCase();
        {
            ...
            DicObj->PropSetByVS(TJS_MEMBERENSURE, __tag_name.AsVariantStringNoAddRef(), &tag_val,
                                DicObj);
        }
...
#define TVP_KAG_STEP_NEXT \
    ...
    { \
        ...
        { \
            TVPAddLog(StorageShortName + TJS_N(" : ignore after multi-line tag : ") + \
                      (&CurLineStr[CurPos + 1])); \
        } \
        CurLine = multiLine + 1; \
        CurPos = 0; \
    ...
    { \
        ...
        while (true)
        {
            ...
            if (CurLineStr[CurPos] == ldelim)
            {
                ...
                if (condition && ExcludeLevel == -1)
                {
                    if (tagkind == tag_endmacro)
                    {
                        ...
                        Macros->PropSet(TJS_MEMBERENSURE, RecordingMacroName.c_str(), NULL,
                                        &macrocontent, Macros);
                    }
                    ...
                    if (RecordingMacro)
                    {
                        if (ldelim != 0)
                        {
                            // normal tag
                            RecordingMacroStr +=
                                ttstr(CurLineStr + tagstart, CurPos - tagstart + 1);
                        }
                        else
                        {
                            // line command
                            if (CurPos - tagstart >= 1)
                            {
                                RecordingMacroStr +=
                                    TJS_N("[") +
                                    ttstr(CurLineStr + tagstart + 1, CurPos - tagstart - 1) +
                                    TJS_N("]");
                            }
                        }
                    ...
                    ismacro =
                        TJS_SUCCEEDED(Macros->PropGet(0, tagname.c_str(), NULL, &macroval, Macros));
                    if (ismacro)
                ...
                if (condition && ExcludeLevel == -1)
                {
                    if (tagkind == tag_emb || (ismacro && tagkind == tag_other))
                    {
                        ...
                        if (!ismacro)
                        {
                            ...
                            tTJSVariant val;
                            ttstr exp;
                            DicObj->PropGet(0, __exp_name.c_str(), __exp_name.GetHint(), &val,
                                            DicObj);
                            exp = val;
                            if (exp == TJS_N(""))
                                TVPThrowExceptionMessage(TVPKAGSyntaxError);
                    ...
                    else if (tagkind == tag_jump)
                    {
                        ...
                        ttstr attrib_target;
                        tTJSVariant val;
                        DicObj->PropGet(0, __storage_name.c_str(), __storage_name.GetHint(), &val,
                                        DicObj);
                        attrib_storage = val;
                        DicObj->PropGet(0, __target_name.c_str(), __target_name.GetHint(), &val,
                                        DicObj);
                        attrib_target = val;
                        ...
                        if (Owner)
                        {
                            ...
                            tTJSVariant res;
                            tjs_error er =
                                Owner->FuncCall(0, event_name.c_str(), event_name.GetHint(), &res,
                                                1, &pparam, Owner);
                            if (er == TJS_S_OK)
                    ...
                    else
                    {
                        ...
                        {
                            tTJSVariant val;
                            DicObj->PropGet(0, TJS_N("name"), 0, &val, DicObj);
                        ...
                        else if (tagkind == tag_erasemacro)
                        {
                            ...
                            DicObj->PropGet(0, TJS_N("name"), 0, &val, DicObj);
                            ttstr macroname = val;
                            if (TJS_FAILED(Macros->DeleteMember(0, macroname.c_str(), 0, Macros)))
            ...
            while (CurLineStr[CurPos] && !TVPIsWS(CurLineStr[CurPos]) &&
                   CurLineStr[CurPos] != TJS_N('=') && CurLineStr[CurPos] != ldelim)
                CurPos++;
            ...
            else
            {
                ...
                if (CurLineStr[CurPos] == TJS_N('\"') || CurLineStr[CurPos] == TJS_N('\''))
                {
                ...
                while (CurLineStr[CurPos] &&
                       (vdelim ? (CurLineStr[CurPos] != vdelim)
                               : (CurLineStr[CurPos] != ldelim && !TVPIsWS(CurLineStr[CurPos]))))
                {
                    if (CurLineStr[CurPos] == TJS_N('`'))
                    {
                        // escaped with '`'
            ...
            if (store)
                DicObj->PropSetByVS(TJS_MEMBERENSURE, attribname.AsVariantStringNoAddRef(),
                                    &ValueVariant, DicObj);
        }
    }
