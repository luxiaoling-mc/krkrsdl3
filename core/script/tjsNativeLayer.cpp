                                      ...
                                      iTJSDispatch2* tjs_obj)
{
    ...
    if (clo.Object == NULL)
        TVPThrowExceptionMessage(TJS_N("Please specify layerTreeOwnerInterface object"));

    class iTVPLayerTreeOwner* lto = NULL;
    tTJSVariant iface_v;
    if (TJS_FAILED(clo.PropGet(0, TJS_N("layerTreeOwnerInterface"), NULL, &iface_v, NULL)))
        TVPThrowExceptionMessage(TJS_N("Cannot Retrive Layer Tree Owner Interface."));
...
void tTJSNI_BaseLayer::Invalidate()
{
    ...
    if (DestSLP)
        DestSLP->Release(), DestSLP = NULL;
    if (SrcSLP)
        SrcSLP->Release(), SrcSLP = NULL;
...
void tTJSNI_BaseLayer::DumpStructure(int level)
{
    ...
    try
    {
        ...
        if (name.IsEmpty())
            name = TJS_N("<noname>");
        tjs_char ptr[80];
        TJS_snprintf(ptr, sizeof(ptr) / sizeof(tjs_char), TJS_N(" (object 0x%p)"), Owner);
        tjs_char ptr2[80];
        TJS_snprintf(ptr2, sizeof(ptr2) / sizeof(tjs_char), TJS_N(" (native 0x%p)"), this);

        TVPAddLog(ttstr(indent) + name + ttstr(ptr) + ttstr(ptr2) + ttstr(TJS_N(" (")) +
                  ttstr(Rect.left) + ttstr(TJS_N(",")) + ttstr(Rect.top) + ttstr(TJS_N(")-(")) +
                  ttstr(Rect.right) + ttstr(TJS_N(",")) + ttstr(Rect.bottom) + ttstr(TJS_N(") (")) +
                  ttstr(Rect.get_width()) + ttstr(TJS_N("x")) + ttstr(Rect.get_height()) +
                  ttstr(TJS_N(")")) + ttstr(TJS_N(" ")) +
                  ttstr(GetVisible() ? TJS_N("visible") : TJS_N("invisible")) + TJS_N(" index=") +
                  ttstr(GetAbsoluteOrderIndex()) + ttstr(ProvinceImage ? TJS_N(" p") : TJS_N("")) +
                  TJS_N(" ") + ttstr(GetTypeNameString()));
    }
    catch (...)
    {
        delete[] indent;
...
{
    if (MainImage)
        delete MainImage, MainImage = NULL;
    if (ProvinceImage)
        delete ProvinceImage, ProvinceImage = NULL;
...
iTJSDispatch2* tTJSNI_BaseLayer::LoadImages(const ttstr& name, tjs_uint32 colorkey)
{
    ...
    try
    {
        ...
        if (!provincename.IsEmpty())
        {
            ...
            try
            {
                TVPLoadGraphicProvince(ProvinceImage, provincename, 0, MainImage->GetWidth(),
                                       MainImage->GetHeight());
...
bool tTJSNI_BaseLayer::GetMostFrontChildAt(
    tjs_int x, tjs_int y, tTJSNI_BaseLayer** lay, const tTJSNI_BaseLayer* except, bool get_disabled)
{
    // internal function
...
//---------------------------------------------------------------------------
tTJSNI_BaseLayer* tTJSNI_BaseLayer::GetMostFrontChildAt(tjs_int x,
                                                        tjs_int y,
                                                        bool exclude_self,
                                                        bool get_disabled)
{
...
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::FireTouchDown(
...
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::FireTouchMove(
    tjs_real x, tjs_real y, tjs_real cx, tjs_real cy, tjs_uint32 id)
{
    if (Owner && !Shutdown)
    {
...
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::FireTouchScaling(
...
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::FireTouchRotate(
...
void tTJSNI_BaseLayer::DefaultKeyDown(tjs_uint key, tjs_uint32 shift)
{
    ...
    bool no_shift_downed =
        !(shift & TVP_SS_SHIFT) && !(shift & TVP_SS_ALT) && !(shift & TVP_SS_CTRL);
...
//---------------------------------------------------------------------------
bool tTJSNI_BaseLayer::ClipDestPointAndSrcRect(tjs_int& dx,
                                               tjs_int& dy,
                                               tTVPRect& srcrectout,
...
//---------------------------------------------------------------------------
bool tTJSNI_BaseLayer::GetBltMethodFromOperationModeAndDrawFace(tTVPBBBltMethod& result,
    ...
    switch (DrawFace)
    {
        ...
        case dfAddAlpha: // additive alpha; main and mask
            ...
            if (opa >= 0)
            {
                color = TVPToActualColor(color);
                ImageModified =
                    MainImage->FillColorOnAddAlpha(destrect, color, opa) || ImageModified;
            }
            else
            {
            ...
            else
            {
                if (ProvinceImage)
                {
                    if (destrect.left == 0 && destrect.top == 0 &&
                        ...
                        destrect.bottom == (tjs_int)ProvinceImage->GetHeight())
                    {
                        // entire area of the province image will be filled with
                        // 0
                        DeallocateProvinceImage();
                        ImageModified = true;
                    }
                    ...
                    {
                        ImageModified =
                            ProvinceImage->Fill(destrect, color & 0xff) || ImageModified;
                    }
                }
            }
            break;
...
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::DrawText(tjs_int x,
                                tjs_int y,
                                const ttstr& text,
                                tjs_uint32 color,
                                tjs_int opa,
                                ...
                                tjs_int shadowofsy)
{
    ...
    MainImage->DrawText(ClipRect, x, y, text, TVP_REVRGB(color), met, opa, HoldAlpha, aa,
                        shadowlevel, TVP_REVRGB(shadowcolor), shadowwidth, shadowofsx, shadowofsy,
                        &r);
...
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::DrawGlyph(tjs_int x,
                                 tjs_int y,
                                 iTJSDispatch2* glyph,
                                 tjs_uint32 color,
                                 tjs_int opa,
                                 ...
                                 tjs_int shadowofsy)
{
    ...
    switch (DrawFace)
    {
        ...
        case dfOpaque:
            ...
            break;
        default:
            TVPThrowExceptionMessage(TVPNotDrawableFaceType, TJS_N("drawGlyph"));
    }
    ...
    MainImage->DrawGlyph(glyph, ClipRect, x, y, color, met, opa, HoldAlpha, aa, shadowlevel,
...
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::PiledCopy(tjs_int dx,
                                 tjs_int dy,
                                 ...
                                 const tTVPRect& srcrect)
{
    ...
    try
    {
        ...
        tTVPRect rc(rect);
        ImageModified = MainImage->CopyRect(dx, dy, bmp, rc, TVP_BB_COPY_MAIN | TVP_BB_COPY_MASK) ||
                        ImageModified;
    }
    catch (...)
    {
...
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::CopyRect(tjs_int dx,
                                tjs_int dy,
                                iTVPBaseBitmap* src,
    ...
    switch (DrawFace)
    {
        ...
        case dfAddAlpha:
            ...
            if (!src)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            ImageModified =
                MainImage->CopyRect(dx, dy, src, rect, TVP_BB_COPY_MAIN | TVP_BB_COPY_MASK) ||
                ImageModified;
            break;
        ...
        case dfMask:
            ...
            if (!src)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            ImageModified =
                MainImage->CopyRect(dx, dy, src, rect, TVP_BB_COPY_MASK) || ImageModified;
            break;
                                   ...
                                   tjs_real typeopt)
{
    ...
    switch (DrawFace)
    {
        ...
        case dfAddAlpha:
            ...
            if (!src)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            ImageModified = MainImage->StretchBlt(ClipRect, destrect, src, srcrect, bmCopy, 255,
                                                  false, type, typeopt) ||
                            ImageModified;
            break;
        ...
        case dfOpaque:
            ...
            if (!src)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            ImageModified = MainImage->StretchBlt(ClipRect, destrect, src, srcrect, bmCopy, 255,
                                                  HoldAlpha, type, typeopt) ||
                            ImageModified;
            break;

        default:
            TVPThrowExceptionMessage(TVPNotDrawableFaceType, TJS_N("stretchCopy"));
    }
                                  ...
                                  bool clear)
{
    ...
    switch (DrawFace)
    {
        ...
        case dfAddAlpha:
        {
            ...
            if (!src)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            updated = MainImage->AffineBlt(ClipRect, src, srcrect, matrix, bmCopy, 255, &updaterect,
                                           false, type, clear, NeutralColor);
            break;
        }
        ...
        case dfOpaque:
        {
            ...
            if (!src)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            updated = MainImage->AffineBlt(ClipRect, src, srcrect, matrix, bmCopy, 255, &updaterect,
                                           HoldAlpha, type, clear, NeutralColor);
            break;
        }

        default:
            TVPThrowExceptionMessage(TVPNotDrawableFaceType, TJS_N("affineCopy"));
    }
...
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::AffineCopy(const tTVPPointD* points,
                                  iTVPBaseBitmap* src,
                                  const tTVPRect& srcrect,
                                  tTVPBBStretchType type,
                                  bool clear)
{
    ...
    switch (DrawFace)
    {
        ...
        case dfAddAlpha:
        {
            ...
            if (!src)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            updated = MainImage->AffineBlt(ClipRect, src, srcrect, points, bmCopy, 255, &updaterect,
                                           false, type, clear, NeutralColor);
            break;
        }
    ...
    switch (DrawFace)
    {
        case dfAlpha:
            ...
            if (!src->MainImage)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            ImageModified =
                MainImage->Blt(dx, dy, src->MainImage, rect, bmAlphaOnAlpha, opacity, HoldAlpha) ||
                ImageModified;
            break;
        ...
        case dfOpaque:
            ...
            if (!src->MainImage)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            ImageModified =
                MainImage->Blt(dx, dy, src->MainImage, rect, bmAlpha, opacity, HoldAlpha) ||
                ImageModified;
            break;
    ...
    switch (DrawFace)
    {
        case dfAlpha:
            ...
            if (!src->MainImage)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            ImageModified =
                MainImage->Blt(dx, dy, src->MainImage, rect, bmCopyOnAlpha, opacity, HoldAlpha) ||
                ImageModified;
            break;
        ...
        case dfOpaque:
            ...
            if (!src->MainImage)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            ImageModified =
                MainImage->Blt(dx, dy, src->MainImage, rect, bmCopy, opacity, HoldAlpha) ||
                ImageModified;
            break;
...
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::OperateRect(tjs_int dx,
                                   tjs_int dy,
                                   iTVPBaseBitmap* src,
                                   const tTVPRect& srcrect,
                                   tTVPBlendOperationMode mode,
                                   ...
                                   tTVPBBStretchType type)
{
    ...
    switch (DrawFace)
    {
        case dfAlpha:
            ...
            if (!src->MainImage)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            ImageModified = MainImage->StretchBlt(ClipRect, destrect, src->MainImage, srcrect,
                                      ...
                                      tjs_real typeopt)
{
    ...
    {
        // unknown blt mode
        TVPThrowExceptionMessage(TVPNotDrawableFaceType, TJS_N("operateStretch"));
    }
    ...
    if (!src)
        TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
    ImageModified = MainImage->StretchBlt(ClipRect, destrect, src, srcrect, met, opacity, HoldAlpha,
                                          type, typeopt) ||
                    ImageModified;
                                  ...
                                  tTVPBBStretchType type)
{
    ...
    switch (DrawFace)
    {
        ...
        case dfOpaque:
        {
            ...
            if (!src->MainImage)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            updated = MainImage->AffineBlt(ClipRect, src->MainImage, srcrect, matrix, bmAlpha,
                                           opacity, &updaterect, HoldAlpha, type);
            break;
        }
        default:
            break;
                                  ...
                                  tTVPBBStretchType type)
{
    ...
    switch (DrawFace)
    {
        ...
        case dfOpaque:
        {
            ...
            if (!src->MainImage)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            updated = MainImage->AffineBlt(ClipRect, src->MainImage, srcrect, points, bmAlpha,
                                           opacity, &updaterect, HoldAlpha, type);
            break;
        }
        default:
                                   ...
                                   tTVPBBStretchType type)
{
    ...
    switch (DrawFace)
    {
        case dfAlpha:
        {
            ...
            if (!src->MainImage)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            updated = MainImage->AffineBlt(ClipRect, src->MainImage, srcrect, matrix, bmCopyOnAlpha,
                                           opacity, &updaterect, HoldAlpha, type);
            break;
        }
        ...
        case dfOpaque:
        {
            ...
            if (!src->MainImage)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            updated = MainImage->AffineBlt(ClipRect, src->MainImage, srcrect, matrix, bmCopy,
                                           opacity, &updaterect, HoldAlpha, type);
            break;
        }
                                   ...
                                   tTVPBBStretchType type)
{
    ...
    switch (DrawFace)
    {
        case dfAlpha:
        {
            ...
            if (!src->MainImage)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            updated = MainImage->AffineBlt(ClipRect, src->MainImage, srcrect, points, bmCopyOnAlpha,
                                           opacity, &updaterect, HoldAlpha, type);
            break;
        }
        ...
        case dfOpaque:
        {
            ...
            if (!src->MainImage)
                TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
            updated = MainImage->AffineBlt(ClipRect, src->MainImage, srcrect, points, bmCopy,
                                           opacity, &updaterect, HoldAlpha, type);
            break;
        }
                                     ...
                                     tTVPBBStretchType type)
{
    ...
    {
        // unknown blt mode
        TVPThrowExceptionMessage(TVPNotDrawableFaceType, TJS_N("operateAffine"));
    }
    ...
    if (!src)
        TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
    updated = MainImage->AffineBlt(ClipRect, src, srcrect, matrix, met, opacity, &updaterect,
                                   HoldAlpha, type);
                                     ...
                                     tTVPBBStretchType type)
{
    ...
    if (!src)
        TVPThrowExceptionMessage(TVPSourceLayerHasNoImage);
    updated = MainImage->AffineBlt(ClipRect, src, srcrect, points, met, opacity, &updaterect,
                                   HoldAlpha, type);
...
{
    if (!MainImage)
        TVPThrowExceptionMessage(TVPUnsupportedLayerType, TJS_N("getTextWidth"));
...
{
    if (!MainImage)
        TVPThrowExceptionMessage(TVPUnsupportedLayerType, TJS_N("getTextHeight"));
...
{
    if (!MainImage)
        TVPThrowExceptionMessage(TVPUnsupportedLayerType, TJS_N("getEscWidthX"));
...
{
    if (!MainImage)
        TVPThrowExceptionMessage(TVPUnsupportedLayerType, TJS_N("getEscWidthY"));
...
{
    if (!MainImage)
        TVPThrowExceptionMessage(TVPUnsupportedLayerType, TJS_N("getEscHeightX"));
...
{
    if (!MainImage)
        TVPThrowExceptionMessage(TVPUnsupportedLayerType, TJS_N("getEscHeightY"));
...
{
    if (!MainImage)
        TVPThrowExceptionMessage(TVPUnsupportedLayerType, TJS_N("getGlyphDrawRect"));
...
void tTJSNI_BaseLayer::QueryUpdateExcludeRect(tTVPRect& rect, bool parentvisible)
{
    ...
    if (parentvisible && (DisplayType == ltOpaque || (MainImage && MainImage->IsOpaque())) &&
        Opacity == 255)
    {
        if (rect.is_empty())
        {
...
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::CopySelfForRect(iTVPBaseBitmap* dest,
                                       tjs_int destx,
                                       tjs_int desty,
                                       const tTVPRect& srcrect)
{
...
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::CopySelf(iTVPBaseBitmap* dest,
                                tjs_int destx,
                                tjs_int desty,
                                const tTVPRect& r)
{
    ...
    else
    {
        ...
        while (it.Step())
        {
            ...
            if (DisplayType != ltBinder)
            {
                tTVPRect pr = cr;
                pr.add_offsets(Rect.left, Rect.top);
                target->DrawCompleted(pr, UpdateBitmapForChild, updaterectforchild, DisplayType,
                                      Opacity);
            }
    ...
    else
    {
        ...
        else
        {
            ...
            while (it.Step())
            {
                ...
                {
                    // copy self image to the target
                    CopySelf(UpdateBitmapForChild, updaterectforchild.left, updaterectforchild.top,
                             cr);
                                     ...
                                     tjs_int opacity)
{
    ...
    if (bmp != UpdateBitmapForChild)
    {
        if (MainImage == NULL)
        {
            ...
            if (DisplayType == type && opacity == 255)
            {
                ...
                while (it.Step())
                {
                    ...
                    UpdateBitmapForChild->CopyRect(r.left - UpdateOfsX, r.top - UpdateOfsY, bmp,
                                                   sr);
                }
            ...
            while (it.Step())
            {
                ...
                BltImage(UpdateBitmapForChild, DisplayType, r.left - UpdateOfsX, r.top - UpdateOfsY,
                         bmp, sr, type, opacity);
            }
        ...
        {
            BltImage(UpdateBitmapForChild, DisplayType, destrect.left - UpdateOfsX,
                     destrect.top - UpdateOfsY, bmp, cliprect, type, opacity);
        }
    }
}
//---------------------------------------------------------------------------
...
// transition management
//---------------------------------------------------------------------------
void tTJSNI_BaseLayer::StartTransition(const ttstr& name,
                                       bool withchildren,
                                       ...
                                       tTJSVariantClosure options)
{
    ...
    try
    {
        ...
        if (TJS_SUCCEEDED(
                options.PropGet(0, selfupdate_name.c_str(), selfupdate_name.GetHint(), &var, NULL)))
        {
            if (var.Type() != tvtVoid)
        ...
        UseTransTickCallback = false;
        if (TJS_SUCCEEDED(
                options.PropGet(0, callback_name.c_str(), callback_name.GetHint(), &var, NULL)))
        {
            // selfupdate member found
            if (var.Type() != tvtVoid)
            {
        ...
        tjs_error er = pro->StartTransition(
            ...
            transsource
                ? (withchildren ? transsource->GetWidth() : transsource->MainImage->GetWidth())
                : 0,
            transsource
                ? (withchildren ? transsource->GetHeight() : transsource->MainImage->GetHeight())
                : 0,
...
void tTJSNI_BaseLayer::InternalStopTransition()
{
    ...
    if (InTransition)
    {
        ...
        if (Owner && !Shutdown && transsrcalive)
        {
            ...
            if (TransDestObj)
                TransDestObj->Release(), TransDestObj = NULL;
            param[1] = tTJSVariant(TransSrcObj, TransSrcObj);
            if (TransSrcObj)
                TransSrcObj->Release(), TransSrcObj = NULL;
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
tTVPBaseTexture* tTJSNI_BaseLayer::tTransDrawable::GetDrawTargetBitmap(const tTVPRect& rect,
                                                                       tTVPRect& cliprect)
{
                                                     ...
                                                     tjs_int opacity)
{
    ...
    if (bmp == Target || Target == NULL)
    {
        ...
        dest = tTVPTempBitmapHolder::GetTemp(cliprect.get_width(), cliprect.get_height(),
                                             true); // fit = true
...
//---------------------------------------------------------------------------
tjs_error tTJSNI_Layer::Construct(tjs_int numparams, tTJSVariant** param, iTJSDispatch2* tjs_obj)
{
...
tTJSNI_Layer* tTJSNI_Layer::FromObject(iTJSDispatch2* obj)
{
    ...
    if (obj)
    {
        if (TJS_FAILED(obj->NativeInstanceSupport(TJS_NIS_GETINSTANCE, tTJSNC_Layer::ClassID,
                                                  (iTJSNativeInstance**)&lay)))
            TVPThrowExceptionMessage(TVPSpecifyLayer);
    }
    return lay;
  ...
  : tTJSNativeClass(TJS_N("Layer")){
        ...
        TJS_DECL_EMPTY_FINALIZE_METHOD
            ...
            TJS_BEGIN_NATIVE_CONSTRUCTOR_DECL(/*var.name*/ _this,
                                              ...
                                              /*TJS class name*/ Layer){return TJS_S_OK;
}
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ setClip) // not setClipRect
{
    ...
    else
    {
        ...
        _this->SetClip((tjs_int)*param[0], (tjs_int)*param[1], (tjs_int)*param[2],
                       (tjs_int)*param[3]);
    }
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ drawText)
{
    ...
    if (numparams < 4)
        return TJS_E_BADPARAMCOUNT;
    _this->DrawText(
        *param[0], *param[1], *param[2], static_cast<tjs_uint32>((tjs_int64)*param[3]),
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ drawGlyph)
{
    ...
    _this->DrawGlyph(
        *param[0], *param[1], glyph, static_cast<tjs_uint32>((tjs_int64)*param[3]),
        (numparams >= 5 && param[4]->Type() != tvtVoid) ? (tjs_int)*param[4] : (tjs_int)255,
        (numparams >= 6 && param[5]->Type() != tvtVoid) ? param[5]->operator bool() : true,
        (numparams >= 7 && param[6]->Type() != tvtVoid) ? (tjs_int)*param[6] : 0,
        (numparams >= 8 && param[7]->Type() != tvtVoid)
    ...
    if (clo.Object)
    {
        ...
        { // try to get bitmap interface
            ...
            if (TJS_FAILED(clo.Object->NativeInstanceSupport(
                    TJS_NIS_GETINSTANCE, tTJSNC_Bitmap::ClassID, (iTJSNativeInstance**)&srcbmp)))
                src = provinceSrc = NULL;
            else
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ copy9Patch)
{
    ...
    if (result)
    {
        ...
        {
            iTJSDispatch2* ret =
                TVPCreateRectObject(margin.left, margin.top, margin.right, margin.bottom);
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ beginTransition)
{
    ...
    if (numparams >= 3 && param[2]->Type() != tvtVoid)
    {
        ...
        {
            if (TJS_FAILED(clo.Object->NativeInstanceSupport(
                    TJS_NIS_GETINSTANCE, tTJSNC_Layer::ClassID, (iTJSNativeInstance**)&transsrc)))
                TVPThrowExceptionMessage(TVPSpecifyLayer);
        }
    }
    if (!transsrc)
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ copyToBitmapFromMainImage)
{
    ...
    {
        if (TJS_FAILED(clo.Object->NativeInstanceSupport(
                TJS_NIS_GETINSTANCE, tTJSNC_Bitmap::ClassID, (iTJSNativeInstance**)&dstbmp)))
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ onSearchPrevFocusable)
{
    ...
    if (param[0]->Type() != tvtVoid)
    {
        ...
        if (clo.Object)
        {
            ...
            {
                if (TJS_FAILED(clo.Object->NativeInstanceSupport(
                        TJS_NIS_GETINSTANCE, tTJSNC_Layer::ClassID, (iTJSNativeInstance**)&src)))
                    TVPThrowExceptionMessage(TVPSpecifyLayer);
            }
            if (!src)
                TVPThrowExceptionMessage(TVPSpecifyLayer);
...
//----------------------------------------------------------------------
TJS_BEGIN_NATIVE_PROP_DECL(provinceImageBufferForWrite){TJS_BEGIN_NATIVE_PROP_GETTER{
    TJS_GET_NATIVE_INSTANCE(/*var. name*/ _this, /*var. type*/ tTJSNI_Layer);
;
...
//----------------------------------------------------------------------
TJS_BEGIN_NATIVE_PROP_DECL(provinceImageBufferPitch){TJS_BEGIN_NATIVE_PROP_GETTER{
    TJS_GET_NATIVE_INSTANCE(/*var. name*/ _this, /*var. type*/ tTJSNI_Layer);
;
