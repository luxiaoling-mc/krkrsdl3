...
extern bool TVPGetBreathing(); // return whether now is in event breathing

extern void TVPSetSystemEventDisabledState(bool en);
/*
        sets whether system overall event handling is enabled.
        this works distinctly from TVPEventDisabled.
*/

extern bool TVPGetSystemEventDisabledState();
//---------------------------------------------------------------------------
...
/*]*/
//---------------------------------------------------------------------------
extern void TVPPostEvent(iTJSDispatch2* source,
...
extern tjs_int TVPCancelEvents(iTJSDispatch2* source,
                               iTJSDispatch2* target,
...
extern bool TVPAreEventsInQueue(iTJSDispatch2* source,
                                iTJSDispatch2* target,
                                const ttstr& eventname,
                                tjs_uint32 tag);
...
extern tjs_int TVPCountEventsInQueue(iTJSDispatch2* source,
                                     iTJSDispatch2* target,
                                     const ttstr& eventname,
                                     tjs_uint32 tag);
...
extern void TVPCancelEventsByTag(iTJSDispatch2* source, iTJSDispatch2* target, tjs_uint32 tag = 0);
// removes all events which have the same source/target/tag.
// tag == 0 removes all tag from queue.

extern void TVPCancelSourceEvents(iTJSDispatch2* source);
...
public:
    tTVPBaseInputEvent(void* source, tjs_int tag)
    {
        Source = source;
        Tag = tag;
    }
...
//---------------------------------------------------------------------------
extern iTJSDispatch2* TVPCreateEventObject(const tjs_char* type,
...
#define TVP_ACTION_INVOKE_BEGIN(argnum, name, object) \
    { \
        ...
        tjs_int arg_count = 0; \
        iTJSDispatch2* evobj = TVPCreateEventObject(TJS_N(name), (object), (object)); \
...
#define TVP_ACTION_INVOKE_END(object) \
    ...
    (object).FuncCall(0, TVPActionName.c_str(), TVPActionName.GetHint(), result, 1, &pevval, \
                      NULL); \
    }
...
/*]*/
//---------------------------------------------------------------------------
extern void TVPAddContinuousEventHook(tTVPContinuousEventCallbackIntf* cb);
extern void TVPRemoveContinuousEventHook(tTVPContinuousEventCallbackIntf* cb);
