...
void tTVPRippleTable::MakeTable()
{
    ...
    try
    {
        ...
        // DriftMap メモリ確保
        // DriftMap に使用するメモリ量は
        // MaxDrift*TVP_RIPPLE_DRIFT_PREC * RippleWidth * 2 * TVP_RIPPLE_DIR_PREC
        // *sizeof(tjs_uint16) tjs_uint32
        // [MaxDrift*TVP_RIPPLE_DRIFT_PREC][RippleWidth*2][TVP_RIPPLE_DIR_PREC] *2 が入っているのは
        // 画像演算中に & でマスクをかける必要がないように
        DriftMap = new tjs_uint16[MaxDrift * TVP_RIPPLE_DRIFT_PREC * RippleWidth * 2 *
                                  TVP_RIPPLE_DIR_PREC];
        ...
        {
            float fdir = M_PI * 0.5 -
                         (((float)w + 0.5) * ((1.0 / (float)TVP_RIPPLE_DIR_PREC) * (M_PI / 2.0)));
            float v;
            v = cos(fdir) * 2048.0;
        ...
        for (drift = 0; drift < MaxDrift * TVP_RIPPLE_DRIFT_PREC; drift++)
        {
            ...
            for (w = 0; w < RippleWidth; w++)
            {
                ...
                for (dir = 0; dir < TVP_RIPPLE_DIR_PREC; dir++)
                {
                    ...
                    tjs_uint16 val = (tjs_uint16)(((int)(char)(int)(xd >> 11) << 8) +
                                                  (int)(char)(int)(yd >> 11));
...
//---------------------------------------------------------------------------
static tTVPRippleTable* TVPGetRippleTable(tjs_int width,
                                          tjs_int height,
                                          tjs_int centerx,
                                          tjs_int centery,
                                          tjs_int ripplewidth,
                                          ...
                                          tjs_int maxdrift)
{
    ...
    for (i = 0; i < TVP_RIPPLE_TABLE_MAX_CACHE; i++)
    {
        ...
        if (table->GetWidth() == width && table->GetHeight() == height &&
            ...
            table->GetRippleWidth() == ripplewidth && table->GetRoundness() == roundness &&
            table->GetMaxDrift() == maxdrift)
        {
            ...
            if (i != 0)
            {
                memmove(TVPRippleTableCache + 1, TVPRippleTableCache, i * sizeof(tTVPRippleTable*));
    ...
    {
        tTVPRippleTable* table = TVPRippleTableCache[TVP_RIPPLE_TABLE_MAX_CACHE - 1];
    ...
    tTVPRippleTable* table =
        new tTVPRippleTable(width, height, centerx, centery, ripplewidth, roundness, maxdrift);
                                   ...
                                   tjs_int ratio)
{
    for (int i = 0; i < num; i++)
    {
        ...
        tjs_intptr_t ofs =
            (int)((i - (int)(char)(n >> 8)) * sizeof(tjs_uint32)) + (int)(char)(n)*pitch;
        TVP_RIPPLE_BLEND
    }
}
//---------------------------------------------------------------------------
                                      ...
                                      tjs_int ratio)
{
    ...
    for (; i < limit; i += 2)
    {
        ...
        tjs_intptr_t ofs1 =
            (int)((i + 0 - (int)(char)(n1 >> 8)) * sizeof(tjs_uint32)) + (int)(char)(n1)*pitch;
        tjs_intptr_t ofs2 =
            (int)((i + 1 - (int)(char)(n2 >> 8)) * sizeof(tjs_uint32)) + (int)(char)(n2)*pitch;
    ...
    if (i < num)
    {
        ...
        tjs_intptr_t ofs1 =
            (int)((i - (int)(char)(n1 >> 8)) * sizeof(tjs_uint32)) + (int)(char)(n1)*pitch;
        __m128i ms2 = _mm_cvtsi32_si128(*(const tjs_uint32*)&src2[ofs1]);
        __m128i ms1 = _mm_cvtsi32_si128(*(const tjs_uint32*)&src1[ofs1]);
        ms2 = _mm_unpacklo_epi8(ms2, mzero);
        ms1 = _mm_unpacklo_epi8(ms1, mzero);
...
static void TVPRippleTransform_sse2_b(const tjs_uint16* displacemap,
                                      const tjs_uint16* driftmap,
                                      tjs_uint32* dest,
                                      tjs_int num,
                                      tjs_int pitch,
...
#ifndef _M_X64
//---------------------------------------------------------------------------
static void TVPRippleTransform_mmx_f(const tjs_uint16* displacemap,
                                     const tjs_uint16* driftmap,
                                     tjs_uint32* dest,
                                     tjs_int num,
                                     tjs_int pitch,
...
//---------------------------------------------------------------------------
static void TVPRippleTransform_mmx_b(const tjs_uint16* displacemap,
                                     const tjs_uint16* driftmap,
                                     tjs_uint32* dest,
                                     tjs_int num,
                                     tjs_int pitch,
...
//---------------------------------------------------------------------------
static void TVPRippleTransform_emmx_f(const tjs_uint16* displacemap,
                                      const tjs_uint16* driftmap,
                                      tjs_uint32* dest,
                                      tjs_int num,
                                      tjs_int pitch,
                                      ...
                                      tjs_int ratio)
{
    ...
    __emit__(0x0f, 0x18, 0x4c, 0x59, 0x10);
    _asm { // prefetcht0	[ecx + ebx*2 + 16]
#else
        }
    _asm _emit 0x0f _asm _emit 0x18 _asm _emit 0x4c _asm _emit 0x59 _asm _emit 0x10 _asm
    { // prefetcht0	[ecx + ebx*2 + 16]
#endif
		psubw		mm1,			mm2 // 1
		movsx		edx,			ah // 2
		pmulhw		mm1,			mm7 // 1
...
//---------------------------------------------------------------------------
static void TVPRippleTransform_emmx_b(const tjs_uint16* displacemap,
                                      const tjs_uint16* driftmap,
                                      tjs_uint32* dest,
                                      tjs_int num,
                                      tjs_int pitch,
                                      ...
                                      tjs_int ratio)
{
    ...
    __emit__(0x0f, 0x18, 0x49, 0xf4);
    _asm { // prefetcht0	[ecx - 12]
#else
        }
    _asm _emit 0x0f _asm _emit 0x18 _asm _emit 0x49 _asm _emit 0xf4 _asm
    { // prefetcht0	[ecx - 12]
#endif
		psllw		mm1,			1 // 1
		movsx		eax,			al // 2
...
#endif
//---------------------------------------------------------------------------
typedef void (*tTVPRippleTransformFunc)(const tjs_uint16* displacemap,
                                        const tjs_uint16* driftmap,
                                        tjs_uint32* dest,
                                        tjs_int num,
                                        tjs_int pitch,
...
#define TVP_RIPPLE_CALC_OFS tjs_intptr_t ofs = x * sizeof(tjs_uint32) + y * pitch;
//---------------------------------------------------------------------------
static void TVPRippleTransform_f_a_e(const tjs_uint16* displacemap,
                                     const tjs_uint16* driftmap,
                                     tjs_uint32* dest,
                                     tjs_int num,
                                     tjs_int pitch,
...
//---------------------------------------------------------------------------
static void TVPRippleTransform_f_d_e(const tjs_uint16* displacemap,
                                     const tjs_uint16* driftmap,
                                     tjs_uint32* dest,
                                     tjs_int num,
                                     tjs_int pitch,
...
//---------------------------------------------------------------------------
static void TVPRippleTransform_b_a_e(const tjs_uint16* displacemap,
                                     const tjs_uint16* driftmap,
                                     tjs_uint32* dest,
                                     tjs_int num,
                                     tjs_int pitch,
...
//---------------------------------------------------------------------------
static void TVPRippleTransform_b_d_e(const tjs_uint16* displacemap,
                                     const tjs_uint16* driftmap,
                                     tjs_uint32* dest,
                                     tjs_int num,
                                     tjs_int pitch,
...
public:
    tTVPRippleTransHandler(tjs_uint64 time,
                           ...
                           tjs_int maxdrift)
    {
        ...
        Table =
            TVPGetRippleTable(Width, Height, CenterX, CenterY, RippleWidth, Roundness, MaxDrift);
    }
    ...
    tjs_error Process(tTVPDivisibleData* data);
...
tjs_error tTVPRippleTransHandler::StartProcess(tjs_uint64 tick)
{
    ...
    Phase =
        (int)(Speed * ((1.0 / (M_PI * 2)) * (1.0 / 1000.0)) * CurTime * RippleWidth) % RippleWidth;
    if (Phase < 0)
        Phase = 0;
    ...
    while (h--)
    {
        ...
        if (y < DriftCarePixels || y >= Height - DriftCarePixels)
        {
            ...
            if (Clip(l, r, data->Left, data->Left + data->Width))
            {
                if (y < CenterY)
                {
                    TVPRippleTransform_b_a_e(
                        Table->GetDisplaceMap(CenterX - l - 1, CenterY - y - 1), CurDriftMap,
                        (tjs_uint32*)dest + l + destxofs, r - l, src1pitch, src1, src2, l, y, Width,
                        Height, BlendRatio);
                }
                else
                {
                    TVPRippleTransform_b_d_e(Table->GetDisplaceMap(CenterX - l - 1, y - CenterY),
                                             CurDriftMap, (tjs_uint32*)dest + l + destxofs, r - l,
                                             src1pitch, src1, src2, l, y, Width, Height,
                                             BlendRatio);
                }
            }
            ...
            if (Clip(l, r, data->Left, data->Left + data->Width))
            {
                if (y < CenterY)
                {
                    TVPRippleTransform_f_a_e(Table->GetDisplaceMap(l - CenterX, CenterY - y - 1),
                                             CurDriftMap, (tjs_uint32*)dest + l + destxofs, r - l,
                                             src1pitch, src1, src2, l, y, Width, Height,
                                             BlendRatio);
                }
                ...
                {
                    TVPRippleTransform_f_d_e(Table->GetDisplaceMap(l - CenterX, y - CenterY),
                                             CurDriftMap, (tjs_uint32*)dest + l + destxofs, r - l,
                                             src1pitch, src1, src2, l, y, Width, Height,
                                             BlendRatio);
                }
        ...
        else
        {
            ...
            if (Clip(l, r, data->Left, data->Left + data->Width))
            {
                ...
                if (Clip(ll, rr, l, r))
                {
                    ...
                    {
                        TVPRippleTransform_b_a_e(
                            Table->GetDisplaceMap(CenterX - ll - 1, CenterY - y - 1), CurDriftMap,
                            (tjs_uint32*)dest + ll + destxofs, rr - ll, src1pitch, src1, src2, ll,
                            y, Width, Height, BlendRatio);
                    }
                    else
                    {
                        TVPRippleTransform_b_d_e(
                            Table->GetDisplaceMap(CenterX - ll - 1, y - CenterY), CurDriftMap,
                            (tjs_uint32*)dest + ll + destxofs, rr - ll, src1pitch, src1, src2, ll,
                            y, Width, Height, BlendRatio);
                    }
                }
            ...
            if (Clip(l, r, data->Left, data->Left + data->Width))
            {
                ...
                if (Clip(ll, rr, l, r))
                {
                    ...
                    if (y < CenterY)
                    {
                        TVPRippleTransform_f(
                            Table->GetDisplaceMap(ll - CenterX, CenterY - y - 1), CurDriftMap,
                            (tjs_uint32*)dest + ll + destxofs, rr - ll, src1pitch,
                            (const tjs_uint8*)((const tjs_uint32*)(src1 + y * src1pitch) + ll),
                            (const tjs_uint8*)((const tjs_uint32*)(src2 + y * src2pitch) + ll),
                            BlendRatio);
                    ...
                    {
                        TVPRippleTransform_f(
                            Table->GetDisplaceMap(ll - CenterX, y - CenterY), CurDriftMap,
                            (tjs_uint32*)dest + ll + destxofs, rr - ll, -src1pitch,
                            (const tjs_uint8*)((const tjs_uint32*)(src1 + y * src1pitch) + ll),
                            (const tjs_uint8*)((const tjs_uint32*)(src2 + y * src2pitch) + ll),
                            BlendRatio);
                ...
                if (Clip(ll, rr, l, r))
                {
                    ...
                    {
                        TVPRippleTransform_f_d_e(Table->GetDisplaceMap(ll - CenterX, y - CenterY),
                                                 CurDriftMap, (tjs_uint32*)dest + ll + destxofs,
                                                 rr - ll, src1pitch, src1, src2, ll, y, Width,
                                                 Height, BlendRatio);
                    }
                }
            }
    ...
    )
    {
        ...
        if ((tjs_uint)maxdrift >= src1w || (tjs_uint)maxdrift >= src1h)
            TVPThrowExceptionMessage(
                TJS_N("maxdrift must be lesser than both image width and height"));

        // オブジェクトを作成
        *handler = new tTVPRippleTransHandler(time, layertype, src1w, src1h, centerx, centery,
