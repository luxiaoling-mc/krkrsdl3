...
//---------------------------------------------------------------------------
tjs_error tTVPBaseRotateTransHandler::Process(tTVPDivisibleData* data)
{
    // トランジションの各領域ごとに呼ばれる
    ...
    while (h--)
    {
        ...
        {
            // 各領域ごとに
            if (line->region[i].left == line->region[i].right)
                continue;
            ...
            if (Clip(l, r, data->Left, data->Left + data->Width))
            {
                ...
                {
                    // src1 または src2
                    const tRotateDrawLine& drawline = (type == 1) ? line->src1 : line->src2;
                    ...
                    if (drawline.stepx == 65536 && drawline.stepy == 0)
                    {
                        ...
                        memcpy((tjs_uint32*)dest + destxofs + l,
                               (const tjs_uint32*)((type == 1 ? src1 : src2) +
                                                   ((drawline.sy +
                                                     (l - drawline.start) * drawline.stepy) >>
                                                    16) *
                    ...
                    {
                        // 拡大縮小転送
                        TVPStretchCopy((tjs_uint32*)dest + destxofs + l, r - l,
                                       (const tjs_uint32*)((type == 1 ? src1 : src2) +
                                                           ((drawline.sy + (l - drawline.start) *
                                                                               drawline.stepy) >>
                                                            16) *
                                                               (type == 1 ? src1pitch : src2pitch)),
                    ...
                    {
                        // 線形変形転送
                        TVPLinTransCopy((tjs_uint32*)dest + destxofs + l, r - l,
                                        (const tjs_uint32*)(type == 1 ? src1 : src2),
                                        drawline.sx + (l - drawline.start) * drawline.stepx,
                                        drawline.sy + (l - drawline.start) * drawline.stepy,
                                        drawline.stepx, drawline.stepy,
                                        type == 1 ? src1pitch : src2pitch);
                    }
                }
            }
...
//---------------------------------------------------------------------------
void tTVPBaseRotateTransHandler::AddLine(tjs_int line, tjs_int left, tjs_int right, tjs_int type)
{
    ...
    if (Clip(left, right, 0, Width))
    {
        ...
        {
            if (data->region[i].left == data->region[i].right)
                continue;
...
void tTVPBaseRotateTransHandler::AddSource(const tPoint* points, tjs_int type)
{
    ...
    if (scanlinestart > points_y[1])
        scanlinestart = points_y[1], toppoint = 1;
    if (scanlineend < points_y[1])
        scanlineend = points_y[1], bottompoint = 1;
    ...
    if (scanlinestart > points_y[2])
        scanlinestart = points_y[2], toppoint = 2;
    if (scanlineend < points_y[2])
        scanlineend = points_y[2], bottompoint = 2;

    // - bottom-left
    points_x[3] = points[2].x;
    points_y[3] = points[2].y;
    if (leftlimit > points_x[3])
        leftlimit = points_x[3];
    if (rightlimit < points_x[3])
        rightlimit = points_x[3];
    if (scanlinestart > points_y[3])
        scanlinestart = points_y[3], toppoint = 3;
    if (scanlineend < points_y[3])
        scanlineend = points_y[3], bottompoint = 3;
    ...
    if (points_y[pdnext] - points_y[pd] + 1)
        pdstepx = 65536 * (points_x[pdnext] - points_x[pd]) / (points_y[pdnext] - points_y[pd] + 1);
    else
        pdstepx = 65536;

    // - for ascent
    if (points_y[panext] - points_y[pa] + 1)
        pastepx = 65536 * (points_x[panext] - points_x[pa]) / (points_y[panext] - points_y[pa] + 1);
    else
        pastepx = 65536;
    ...
    if (points_y[pdnext] - points_y[pd] + 1)
    {
        ...
        {
            case 0:
                sdstep = 65536 * (Height) / (points_y[pdnext] - points_y[pd] + 1);
                break;
            case 1:
                sdstep = 65536 * (-Width) / (points_y[pdnext] - points_y[pd] + 1);
                break;
            case 2:
                sdstep = 65536 * (-Height) / (points_y[pdnext] - points_y[pd] + 1);
                break;
    ...
    if (points_y[panext] - points_y[pa] + 1)
    {
        ...
        {
            case 0:
                sastep = 65536 * (Width) / (points_y[panext] - points_y[pa] + 1);
                break;
            case 1:
                sastep = 65536 * (Height) / (points_y[panext] - points_y[pa] + 1);
                break;
            case 2:
                sastep = 65536 * (-Width) / (points_y[panext] - points_y[pa] + 1);
                break;
    ...
    if (points_y[pd] < y)
        pdx += pdstepx * (y - points_y[pd]);

    // - for ascent
    pax = points_x[pa] * 65536;
    if (points_y[pa] < y)
        pax += pastepx * (y - points_y[pa]);
    ...
    for (; y <= ylim; y++)
    {
        ...
        if (adx < ddx)
            Swap_tjs_int(dx, ax), Swap_tjs_int(dy, ay), Swap_tjs_int(adx, ddx);
        ...
        tRotateDrawLine& drawline = (type == 1) ? DrawData[y].src1 : DrawData[y].src2;
