...
static const tjs_int gloss[64] = {
    0, 0, 0, 0, 16, 48, 80, 128, 192, 128, 80, 48, 16, 0, 0, 0, 0, 0, 0, 0, 0, 0,
...
tjs_error tTVPTurnTransHandler::StartProcess(tjs_uint64 tick)
{
    ...
    Phase =
        CurTime * (64 + (xcount + ycount) * TURN_WIDTH_FACTOR) / Time - ycount * TURN_WIDTH_FACTOR;
...
//---------------------------------------------------------------------------
tjs_error tTVPTurnTransHandler::Process(tTVPDivisibleData* data)
{
    // トランジションの各領域ごとに呼ばれる
            ...
            if (y * 64 + 64 >= Height || x == startx || x == endx)
            {
                    ...
                    Clip(t, b, data->Top, data->Top + data->Height))
                {
                    ...
                    if (phase == 0)
                    {
                        // 完全に src1
                        tjs_uint8* dp =
                            dest + (t + destyofs) * destpitch + (l + destxofs) * sizeof(tjs_uint32);
                        const tjs_uint8* sp = src1 + t * src1pitch + l * sizeof(tjs_uint32);
                        tjs_int count = b - t;
                    ...
                    else if (phase == 63)
                    {
                        ...
                        tjs_uint8* dp =
                            dest + (t + destyofs) * destpitch + (l + destxofs) * sizeof(tjs_uint32);
                        const tjs_uint8* sp = src2 + t * src2pitch + l * sizeof(tjs_uint32);
                        tjs_int count = b - t;
                        tjs_int len = (r - l) * sizeof(tjs_uint32);
                        while (count--)
                    ...
                    else
                    {
                        ...
                        tjs_int count = b - t;
                        tjs_uint8* dp =
                            (tjs_uint8*)((tjs_uint32*)(dest + (t + destyofs) * destpitch) + x * 64 +
                                         destxofs);
                        while (count--)
                        {
                            tjs_int fl, fr;
            ...
            else
            {
                ...
                if (Clip(t, b, data->Top, data->Top + data->Height))
                {
                    ...
                    else
                    {
                        ...
                        tjs_int count = b - t;
                        tjs_uint8* dp =
                            (tjs_uint8*)((tjs_uint32*)(dest + (t + destyofs) * destpitch) + l +
                                         destxofs);
                        while (count--)
                        {
                            tjs_int fl, fr;
                            ...
                            if (gl)
                            {
                                for (; fl < fr; fl++)
                                {
                                    ((tjs_uint32*)dp)[fl] = Blend(
                                        *(const tjs_uint32*)(src + (sx >> 16) * sizeof(tjs_uint32) +
                                                             (sy >> 16) * srcpitch),
                                        0xffffff, gl);
                                    sx += params->stepx;
                                    sy += params->stepy;
                            ...
                            else
                            {
                                TVPLinTransCopy((tjs_uint32*)dp + fl, fr - fl,
                                                (const tjs_uint32*)src, sx, sy, params->stepx,
                                                params->stepy, srcpitch);
                                /*
                                                                                                for(;
                                   fl < fr; fl++)
                                                                                                {
