...
public:
    tTVPWaveTransHandler(tjs_uint64 time,
    ...
    for (n = 0; n < data->Height; n++, rad += CurOmega)
    {
        ...
        if (Clip(l, r, data->Left, data->Left + data->Width))
        {
            if (LayerType == ltAlpha)
                TVPConstAlphaBlend_SD_d(dest + l + data->DestLeft - data->Left, src1 + l - d,
                                        src2 + l - d, r - l, BlendRatio);
            else if (LayerType == ltAddAlpha)
                TVPConstAlphaBlend_SD_a(dest + l + data->DestLeft - data->Left, src1 + l - d,
                                        src2 + l - d, r - l, BlendRatio);
            else
                TVPConstAlphaBlend_SD(dest + l + data->DestLeft - data->Left, src1 + l - d,
                                      src2 + l - d, r - l, BlendRatio);
            /*
